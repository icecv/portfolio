...
这项技术确实令人兴奋。但真正困难、真正重要的工作，是让它可靠到足以让人真正依赖。
  ...
  {
    id: '4',
    slug: 'rag-optimization-loop',
    title: '从"能用"到"好用"：我如何理解 RAG 系统的优化闭环',
    description:
      '从最初把 RAG 当作"数据外挂"的浅层认知，到理解检索精度、数据质量、反馈闭环三条优化主线——以及为什么用薯条工厂来类比 RAG Pipeline 意外地准确。',
    date: '2026-04-23',
    tags: ['RAG', 'AI 系统', '技术理解', '产品思考'],
    readingTime: '6 分钟',
    content: `
## 最开始，我对 RAG 的理解很浅
...
**数据清洗**是第一步：过滤掉噪声信息、去除重复内容、统一格式。很多人跳过这步，直接把原始文档塞进向量库，然后疑惑为什么检索结果一塌糊涂。
...
土豆进入工厂，首先被**切分**成条状（Chunking），然后按照**大小相似性**进行匹配分类（向量化），再通过**孔洞筛选**确保形状一致（Rerank 过滤），最后把**形状最好的几根**送进名为 LLM 的油锅，炸出最符合用户口味的薯条。
